...
fn re_signed_email_yields_different_nullifier_both_accepted() {
    ...
    //
    // resigned_a.eml / resigned_b.eml: identical bodies signed by the same
    // synthetic domain+key (insider.test / v0test) over different Date
    // headers, so each carries a distinct `b=`. v0 computes
    // nullifier = Poseidon(domain_sep, claimed_domain, signature), so the
    // two distinct signatures must yield two distinct nullifiers, and both
    // verify against the same pinned pubkey => both accepted as separate
    // events. Fixtures are generated by tests/fixtures/gen_resigned.py.
    let tmp = tempfile::tempdir().unwrap();
    let store = tmp.path().join("nullifiers.txt");
    let tag = std::fs::read_to_string(fixture("resigned.pubkey.tag"))
        .expect("fixture resigned.pubkey.tag missing")
        .trim()
        .to_string();

    let nullifier_of = |label: &str| -> String {
        let email = fixture(&format!("resigned_{label}.eml"));
        let proof = tmp.path().join(format!("{label}.bin"));
        let (ok, _out, err) = run_prove(&email, &proof, &tag, &[]);
        assert!(ok, "prove resigned_{label} failed:\n{err}");
        let (ok, out, err) = run_verify(&proof, &store);
        assert!(ok, "verify resigned_{label} rejected:\n{out}{err}");
        assert!(out.contains("ACCEPTED"), "expected ACCEPTED for {label}:\n{out}");
        out.lines()
            .find_map(|l| l.strip_prefix("Nullifier: "))
            .expect("verify output missing Nullifier line")
            .trim()
            .to_string()
    };

    let na = nullifier_of("a");
    let nb = nullifier_of("b");
    assert_ne!(
        na, nb,
        "re-signed email (different b=) must yield a distinct nullifier"
    );
}
