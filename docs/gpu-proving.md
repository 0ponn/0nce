...
> **Where the `cuda` feature lives.** It is intentionally **not on `main`** — enabling
> `risc0-zkvm/cuda` drags RISC0's Groth16-CUDA subtree (~161 crates) into `Cargo.lock`,
> and 0nce produces STARK receipts (no Groth16), so it would be dead weight in the
> default dependency graph. The feature + lock changes live on branch
> **`feat/gpu-proving`**. To use it: `git checkout feat/gpu-proving`, or add
> `cuda = ["risc0-zkvm/cuda"]` under `[features]` in `host/Cargo.toml` locally.
